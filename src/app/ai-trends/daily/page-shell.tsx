...
export function generateAiTrendDailyArchiveMetadata(): Metadata {
  const title = buildMetadataTitle({ pageTitle: "AI 需求趋势每日晨报", brand: siteName });
  const description = buildMetaDescription("AI 需求趋势每日晨报归档。公开用户可阅读摘要，登录用户可阅读完整 HTML 报告。");
  ...
  return {
    ...
    openGraph: {
      ...
      siteName,
      images: [{ url: absoluteUrl(defaultOgImage), alt: "AI 需求趋势每日晨报" }],
      locale: "zh_CN",
      type: "website"
    }
  };
  ...
  return (
    <Container className="py-14">
      <section className="surface-panel p-7 md:p-10">
        <Badge className="text-[var(--marketing-accent)]">Daily Briefing</Badge>
        <h1 className="mt-5 text-4xl font-black leading-tight text-[var(--marketing-text)] md:text-5xl">AI 需求趋势每日晨报</h1>
      ...
      <section className="mt-12">
        <SectionTitle title="晨报归档" intro="这些日更页用于用户分享和阅读，不进入 sitemap，并通过 robots metadata 设置 noindex, follow。" />
        {briefings.length ? (
          ...
          <EmptyState title="暂无晨报" text="自动化发布第一份 AI 需求趋势晨报后，这里会显示公开摘要。" />
        )}
      </section>
    </Container>
  );
