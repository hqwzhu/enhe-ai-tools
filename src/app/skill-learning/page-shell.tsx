...
import { buildBreadcrumbSchema, buildFaqSchema, buildListingMetaDescription, buildLocalePath, buildMetadataTitle, buildPageMetadata } from "@/lib/seo";
  ...
  zh: [
    {
      ...
      answer: "适合。建议从提示词基础、常用 AI 工具工作流和具体任务案例开始，再逐步学习本地部署、自动化和多模态创作。"
    },
    {
      question: "如何判断应该先学课程还是先选工具？",
      answer: "如果你已经有明确任务，可以先选工具再按教程落地；如果还不清楚 AI 能解决什么问题，先看 AI 前沿资讯和趋势分析，再选择课程。"
    },
    {
      question: "AI 技能课程如何转化为实际工作成果？",
      answer: "围绕一个真实交付物学习，例如一份方案、一组视频素材、一个自动化流程或一套运营模板，并把步骤沉淀为可复用流程。"
    }
  ],
  en: [
    {
      question: "Is ENHE AI skill learning suitable for beginners?",
      answer: "Yes. Start with prompt basics, common AI tool workflows, and task-based examples, then move into local deployment, automation, and multimodal creation."
    },
    {
      question: "Should I choose a tool or a course first?",
      answer: "If you already have a task, choose the tool first and follow a tutorial. If you are exploring opportunities, read AI news and trend analysis before choosing a course."
    },
    {
      question: "How do AI courses become real work outcomes?",
      answer: "Learn around one deliverable, such as a plan, video asset, automation workflow, or operations template, then save the process as a reusable workflow."
    }
  ]
} as const;
...
export async function SkillLearningPageShell({
  ...
  const faqSchema = buildFaqSchema({ items: [...skillLearningFaqItems[forceLocale]] });
    ...
    <section className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="surface-panel-soft p-6">
        <h2 className="text-2xl font-black text-[var(--marketing-text)]">
          {forceLocale === "en" ? "From AI skills to repeatable workflows" : "从 AI 技能到可复用工作流"}
        </h2>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {sections.map((section, index) => (
            <article key={section.title} className="rounded-2xl border border-white/10 bg-white/7 p-4">
              <span className="text-xs font-black uppercase tracking-[0.22em] text-[var(--marketing-accent)]">
                {String(index + 1).padStart(2, "0")}
              </span>
              <h3 className="mt-3 text-base font-black leading-snug text-[var(--marketing-text)]">{section.title}</h3>
              <p className="mt-2 text-sm leading-7 text-[var(--marketing-muted)]">{section.body}</p>
            </article>
          ))}
        </div>
      </div>
      <div className="surface-panel-soft p-6">
        ...
        <div className="mt-5 space-y-4">
          {faqs.map((item) => (
            <article key={item.question} className="rounded-2xl border border-white/10 bg-white/7 p-4">
              <h3 className="text-base font-black leading-snug text-[var(--marketing-text)]">{item.question}</h3>
              <p className="mt-2 text-sm leading-7 text-[var(--marketing-muted)]">{item.answer}</p>
            </article>
          ))}
        </div>
      </div>
