  ...
  return (
    <Container className="py-14">
            ...
            <div>
              <div className="flex flex-wrap gap-2">
                ...
                <Badge className={(isSkillLearning && !hasDownloadPurchase) || (tool.type === "software" && tool.isDownloadPaid) || (isAccountService && servicePrice > 0) ? "text-[#FFB86B]" : "text-[#5EF1C7]"}>
                  {isSkillLearning
                    ? (forceLocale === "en" ? "Paid course" : "收费服务")
                    : tool.type === "software" && tool.isDownloadPaid
                      ? (forceLocale === "en" ? "Paid software" : "收费软件")
                ...
                {shouldShowPurchaseForm ? (
                  <form id={isAccountService ? "service-purchase" : "download-purchase"} action={createSoftwareDownloadOrderAction} className="grid w-full gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <FormSubmitButton pendingLabel={forceLocale === "en" ? "Generating payment QR..." : "生成支付二维码中..."}>
                        {isAccountService ? td.buyService : isSkillLearning ? (forceLocale === "en" ? "Purchase now" : "点击购买") : td.buyDownload.replace("{price}", Number(servicePrice).toFixed(2))}
                      </FormSubmitButton>
                    </div>
                  </form>
                ) : tool.type === "software" ? (
                  ...
                  visibleChangelogs.slice(0, 3).map((item) => (
                    <div key={item.id} className="rounded-2xl border border-white/10 bg-white/8 p-4">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {item.releaseDate ? <p className="text-xs text-[#8F9DB2]">{item.releaseDate.toLocaleDateString(forceLocale === "en" ? "en-US" : "zh-CN")}</p> : null}
                      </div>
        ...
        <section id="tool-intro" className="glass scroll-mt-24 rounded-2xl p-7">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <TrustItem label={td.supportEmail}>
              ...
              <Link href={buildLocalePath("/legal/membership-refund", forceLocale)} className="mt-2 block text-sm leading-6 text-[#FFB86B] hover:text-[#FFD29B]">
                {td.refundRulesIntro}
              </Link>
            </TrustItem>
          </div>
        ...
        <section className="glass rounded-2xl p-7">
          ...
          {isSkillLearning && !hasDownloadPurchase ? (
            <div className="rounded-2xl border border-[#FFB86B]/25 bg-[#FFB86B]/8 p-6 text-center">
              <p className="text-sm font-semibold text-[#FFB86B]">{forceLocale === "en" ? "Please purchase this course to view the content" : "请先购买课程后查看课程内容"}</p>
            </div>
          ) : (
        ...
        <section className="glass rounded-2xl p-7">
          ...
          {user ? (
            <form action={createCommentAction} className="mb-6">
              ...
              <FormSubmitButton className="mt-3 text-base" pendingLabel={forceLocale === "en" ? "Submitting..." : "提交中..."}>{td.submitComment}</FormSubmitButton>
            </form>
          ) : (
      ...
      <section className="mt-12">
        ...
        <div className="grid gap-5 md:grid-cols-3">{related.map((item) => <ToolCard key={item.id} tool={item} locale={forceLocale} />)}</div>
      </section>
    </Container>
  );
}
