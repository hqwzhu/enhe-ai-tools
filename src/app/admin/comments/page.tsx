      ...
      <div className="mt-8 space-y-4">
        {comments.map((comment) => (
          <div key={comment.id} className="glass rounded-2xl p-6">
            <p className="text-sm text-[#8B95A7]">{comment.tool.name} · {comment.user.email} · {comment.status}</p>
            <p className="mt-3 leading-7">{comment.content}</p>
            <form action={updateCommentStatusAction} className="mt-5 flex flex-wrap gap-3">
